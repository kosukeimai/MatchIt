nearest <-  function(formula, data, model="logit", 
  ...
  if (m.order==1) {
   ...
   #################################################################################
   ## Calculate weights
    match.matrix2 <- match.matrix[in.sample[treat==1],,drop=F]
    num.matches <- dim(match.matrix2)[2]-apply(as.matrix(match.matrix2), 1, function(x) { sum(is.na(x)) })
    names(num.matches) <- tlabels[in.sample[treat==1]]
    
    t.units <- row.names(match.matrix2)[num.matches>0]
    c.units <- na.omit(as.vector(as.matrix(match.matrix2)))
    ...
    for (cont in clabels) {
      treats <- na.omit(row.names(match.matrix2)[cont==match.matrix2[,1]])
      if (dim(match.matrix2)[2]>1) {
        for (j in 2:dim(match.matrix2)[2])
          treats <- c(na.omit(row.names(match.matrix2)[cont==match.matrix2[,j]]),treats)
      }
     }
