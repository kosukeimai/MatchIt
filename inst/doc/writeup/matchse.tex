...
speaking, hypothesis tests are irrelevant in this context \citep{Hansen06}.  Virtually
...
$G$, when multiplied by $\gamma$, will generate arbitrarily large
...
without special assumptions \citep{RubStu06}, and so proper measures of imbalance
should always be minimized without limit, subject to efficiency
constraints.  Thus, whether or not some hypothesis test indicates that
$G$ is not significantly different from zero is immaterial: The
smaller $G$ is the better, either above or below the $t$-test
threshold of significance, since $G$ (i.e., balance) is a
...
statistics \citep{AusMam06, Hansen04, Rubin01}. A more general approach is quantile-quantile (or ``QQ'')
