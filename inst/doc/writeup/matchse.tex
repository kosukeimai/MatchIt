...
Second, we offer a simple model that conveys why matching contains no
...
Third, the problem in Figure~\ref{f:randrop} can be seen by
...
$\overline{X}_{mc}=\sum_{i=1}^n_m (1-T_i)X_i/n_{mc}$ are the sample means, and
$s^2_{mt}=\sum_{i=1}^n_m T_i(X_i - \overline{X}_{mt})^2/(n_{mt}-1)$ and
$s^2_{mc}=\sum_{i=1}^n_m (1-T_i)(X_i - \overline{X}_{mt})^2/(n_{mc}-1)$ represent
...
number of remaining observations $n_m$, (2) the ratio of remaining
treated units to the total number of remaining observations $r_m$, and
(3) the sample variance of $X$ for the remaining treated and control
units, $s_{mt}^2$ and $s_{mc}^2$.  Since the value of (this and other)
...
Finally, from a theoretical perspective, balance is a characteristic of
...
the left graph.\footnote{Formally, this measure can be defined as
  ...
  treatment and matched control groups, respectively, and $n=min{n_{mt},n_{mc}}$.}  Unlike the
