...
\paragraph{formalization 1.} Suppose 
...
\begin{eqnarray*}
  ...
  = 0) - \widetilde{F}(X \mid T = 1)]  \\
  & & + \frac{1}{2} \left\{\int U_1 \,d[\widetilde{F}(U_1 \mid T = 0) -
  \widetilde{F}(U_1 \mid  T = 1)] + \int U_0
  \,d[\widetilde{F}(U_0 \mid T = 0) -  \widetilde{F}(U_0 \mid  T = 1)]
  \right\}    
\end{eqnarray*}
...
\paragraph{formalization 3.} Let $I_i$ be a sampling indicator where $I_i=1$
implies unit $i$ is in the sample. Suppose that our estimand is the
average treatment effect and our estimator is the difference in sample
means. Then,
\begin{eqnarray*}
 & & {\rm Discrepancy} \\
 & = &  E[Y_i(1) - Y_i(0)] - \left\{\frac{1}{n} \sum_{i \, \in \{I_i
 = 1, T_i =1 \}}
  Y_i(1) - \frac{1}{n} \sum_{i \, \in \{I_i = 1, T_i =0 \}}
  Y_i(0) \right\}, \\
& = & E[Y_i(1) - Y_i(0) \mid I_i = 1] \Pr(I_i = 1) + E[Y_i(1) - Y_i(0)
 \mid I_i = 0] [1-\Pr(I_i = 1)]  \\
& & 
- \left\{\frac{1}{n} \sum_{i \, \in \{I_i
 = 1, T_i =1 \}}
  Y_i(1) - \frac{1}{n} \sum_{i \, \in \{I_i = 1, T_i =0 \}}
  Y_i(0) \right\}, \\
& = & [1-\Pr(I_i = 1)] \left\{E[Y_i(1) - Y_i(0) \mid I_i = 0] -E[Y_i(1) -
 Y_i(0) \mid I_i = 1] \right\} \\
& & + E[Y_i(1) - Y_i(0) \mid I_i = 1] - \left\{\frac{1}{n} \sum_{i \, \in \{I_i
 = 1, T_i =1 \}}
  Y_i(1) - \frac{1}{n} \sum_{i \, \in \{I_i = 1, T_i =0 \}}
  Y_i(0) \right\},
\end{eqnarray*}
Notice that the first term is the {\it discrepancy due to sample
  selection}, which is no zero unless either $\Pr(I_i =1)$, i.e.,
everyone is in the sample, or $E[Y_i(1) - Y_i(0) \mid I_i =
0]=E[Y_i(1) - Y_i(0) \mid I_i = 1]$, i.e., random sample. Now, the
second term can be written as,
\begin{eqnarray*}
 & &  \left\{E[Y_i(1) - Y_i(0) \mid I_i = 1] - \frac{1}{2n}
  \sum_{i\,\in \{I_i = 1\}} [Y_i(1) -  Y_i(0)] \right\} + 
\\
&  &\left\{\frac{1}{2n} \sum_{i=1}^{2n} [Y_i(1) -
  Y_i(0)] - \left[\frac{1}{n} \sum_{i \, \in \{T_i =1 \}}
  Y_i(1) - \frac{1}{n} \sum_{i \, \in \{T_i =0 \}}
  Y_i(0) \right]\right\}, 
\end{eqnarray*}
where the first term can be called as ``finite sample discrepancy''
and is equal to
\begin{equation*}
  \int I_i(Y_i(1) - Y_i(0))\; [ dF(Y_i(1), Y_i(0) \mid I_i = 1)
  - d\widetilde{F}(Y_i(1), Y_i(0) \mid I_i = 1)],
\end{equation*}
the second term equal the discrepancy expression in the
``formalization 2.'' 
