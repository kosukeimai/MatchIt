...
level of balance, and so we needed propensity scores with
subclassification.  For pedagogical reasons, and to save space, we use
different methods of evaluating (\ref{balance}) in our two
applications.
...
Administration (FDA) (we concentrate on Model 1 of Table 2).  Here, we
focus on the hypothesis that ``expected approval times are greater
when Democrats control the White House, when the agency's oversight
...
overseers are, the more likely the slower the FDA approves new drugs.

To test this hypothesis, Carpenter uses a lognormal survival model of
approval times regress on his primary causal variables (median
adjusted ADA scores for House and Senate Committees as well as for
House and Senate floors, Democratic Majority in House and Senate, and
Democratic Presidency) and 18 control variables including clinical and
epidemiology factors and firm characteristics.\footnote{In the
  original paper, \citet{Carp02} uses a lognormal frailty model with a
  common (ungrouped) random effect.  Here, for simplicity and speed of
  computation, we drop the random effect.  This has very small effects
  on the quantities we estimate, and no effect on our conclusions.}
The dataset consists of 408 new drugs whose applications were reviewed
...
hence are treated as right-censored observations. The approval time is
...
the binary treatment variable of the existence of a Democratic Senate
majority, which is one of the seven oversight variables. In the
original analysis, the reported coefficient for this variable is
statistically significant at 10\% level but in the opposite direction
of Carpenter's hypothesis.  For our purposes, however, this treatment
variable is of particular interest because \citet[p.498][]{Carp02}
finds that ``The coefficient estimate for this variable [Democratic
Senate majority] is not significant in other regressions, and even
switches sign when firm variables are added (analyses available upon
request)''. We are, therefore, interested in how this model
sensitivity can be reduced by preprocessing the data.  (We omit the
remaining six oversight variables because they are conceptually and
statistically highly related, and in part consequences of, the
Democratic Senate majority variable.)
...
that including a square root of the nightly TV news disease story
...
the treatment and control groups.  However, after matching, these four
...
\begin{figure}[t] 
 \begin{center}
  ...
  \end{center}
  \vspace{-0.275in}
  \caption{Kernel density plot (a smooth version of a histogram)
...
it with the results from the raw data.  To do this, we estimate the
...
potential specifications of pre-treatment covariates, although the
...
(i.e., all possible subsets of covariates from the 18).  Although this
includes $262,143$ survival analyses, even it omits interactions and
nonlinearities.  Figure~\ref{fg:fdadens} presents the density plot for
the estimated average treatment effects for the raw data as well as
the matched (preprocessed) data.  The estimates are far more model
...
that the estimate is much less.  Indeed, Carpenter notes the large
model sensitivity...  However, the result from the matched data
indicate that the effect size may actually be larger.
...
of no interactions and no nonlinearities.
...
propensity score equation.  However, subclassifing on the propensity
...
involves a logistic regression of visibilty on all six pre-treatment
...
Indeed across subclass-covariates the t-statistic exceeds 2 only once,
which is roughly the same as we would expect if the data were
generated randomly.
\begin{figure}[t] 
 \begin{center}
