...
various threats to the validity of their causal
inferences.\footnote{Our definition of ``observational data,'' which
  includes any data set that requires at least one unverifiable
...
within each pair of units.\footnote{Formally, ignorability, by which
  we mean the independence of $T$ and the potential outcomes given $X$, and
  balance in Equation \ref{balance} guarantee valid inferences, which
  can be written as $P(Y(0)|T=1)=P(Y(0)|T=0)$ and
  $P(Y(1)|T=1)=P(Y(1)|T=0)$.}
...
of analysis, it is not necessary to break the link between $t$ and
...
therefore now turn in Section \ref{s:choose} to a menu of matching
...
assumptions and so will return highly similar causal effect
estimates.
...
nearest neighbor matching on the propensity score).\footnote{When
  ...
  invariant to the order in which we match units.  A second approach
  minimizes the total distance within matched units \citep[][pp.
  311-23]{Rosenbaum02}.  Our software implements both, incorporating
  optimal matching code provided by \citet{Hansen03}.} If this
procedure balances $X$ (and thus satisfies the procedures for checking
balance we describe in Step 4), we use it.  If not, then we respecify
the logistic regression by adding interactions or squared terms and
match again.  If that works, then we use it.  If not, we can try even
more elaborate specifications (when necessary, perhaps even trying
other functional forms such as CART, neural network analyses, or
others).
...
them in the propensity score but not in the Mahalonobis distance
calculation.
...
frequently have good qualitative knowledge not coded in $X$
