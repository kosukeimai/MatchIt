...
The notation and ideas in this section parallel that in \citet[][Section
...
in experimental and observational research designs.  Some version of
these assumptions, or some way to deal with the information in them,
are necessary no matter what statistical methods are used for
estimation.  Any specific statistical method chosen will make other
assumptions, but the ones discussed in this section affect essentially
all methods.
...
dependent variable only by random chance.  Combining this with the
large $n$ from the third feature guarantees that the chance that
something will go wrong is vanishingly small.
...
literature.  We mention it here to emphasize that all the well-known
...
We also assume that a researcher has sufficient information in their
measured pre-treatment control variables $X$ so that it is possible
via \emph{some} method to make valid causal inferences.  This is known
in some fields as the absence of omitted variable bias, so that $X_i$
...
many assumptions to be made by individual methods of statistical
inference.  Results from these methods can still be biased, unbiased,
efficient, inefficient, and almost anything else, depending on the
analyst's choices.  We now focus on this point in the context of
...
other models.  For some examples, if $p(\cdot)$ is normal and
$g(c)=c$, we have linear regression.  If $p(\cdot)$ is Bernoulli and
$g(c)=1/(1+e^{-c})$, we have logit.
...
independence implies mean independence, we can write
...
\end{equation}
which is one number and without a summation sign over $i$.

The systematic components of both equations of (\ref{smatt}) are now
scalar constants for all $i$.  This is a key result, since it means
that changing the definition of the functional form $g(\cdot)$ now
amounts to nothing more than a simple reparameterization.
Fortunately, maximum likelihood is invariant to reparameterization
...
control groups.  But even if one chooses to run a parametric model, it
will not matter: the results will be the same no matter what the
choice for $g()$ is.
...
analyzing observatonal data with parametric methods we are not so
fortunate.  We cannot reduce Equations \ref{att} to Equations
...
control variable is misspecified.
...
know that 97 parameters should be set to exactly zero?  Including a
...
term and five slope coefficients and restricting by assumption the
remaining 99,994 others.  Nine such explanatory variables has a model
with one \emph{billion} parameters, which would be approximated by
only the ten that we would estimate under the traditional approach.
Eighty ten-category explanatory variables would require a regression
model with more parameters than current estimates of the number of
elementary particles in the universe.

Estimating rather than making assumptions about all these extra
parameters is not possible under the standard regression approach,
since our data sets do not have sufficient observations to support
estimation of anywhere near the required number of parameters.  We
cannot avoid the problem wtih nonlinear or non-normal statistical
models, since these pose the same curse of dimensionality as linear
regression.  Instead, many assumptions about many parameters are
necessary under this approach.  The unavoidable consequence is high
levels of model dependence:  change the assumptions 

\section{Matched Preprocessing}

Also, just a quick point, but I wanted to mention that I think it's also
important to note that if a bunch of fairly reasonable different models do
give wildly different answers after matching, then it's probably a sign
that the data isn't there to support causal inference, and I probably
