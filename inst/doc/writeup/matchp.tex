...
\begin{figure}[t] 
 \begin{center}
  ...
  \caption{Dangers in relying on t-statistics as measure of balance.
    This figure illustrates the dangers in using t-statistics, which involve the
    sample size, as a measure of balance.  Each panel shows the effect of changing
    sample size on a measure of balance.  The lines indicate the average value of
    the statistic when a given number of control units are randomly dropped from the data
    set (out of a total of 245 control units).  With increasing numbers of control units dropped, the t-statistic gets 
    closer to 0, falsely indicating reductions in balance with the smaller resulting sample sizes.  In fact, because 
    controls are
    dropped randomly, the difference in means stays the same, as indicated in the second panel.}
  \label{fg:tstat}
\end{figure}
