...
We have also assumed that all treatments are the same.  This
assumption would be violated in our running example if incumbency
status meant something different across districts.\footnote{These last
...
nearest neighbor matching on the propensity score).\footnote{When
  ...
  invariant to the order in which units are matched.  A second
  approach, known as ``optimal'' matching, minimizes the total
...
then we use it.  If not, we can try even more elaborate specifications
(when necessary, perhaps even trying other functional forms such as
CART, neural network analyses, or others).
