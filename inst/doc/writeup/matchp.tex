...
only arises in private and so {\bf remove ``so''} open discussions of
it are only infrequently {\bf remove only and replace infrequently
  with rarely} found in the literature.
...
seconds, and according to some textbook ways of proceeding {\bf this
  part is a bit awkward}, it would be time to write up the results.
...
different causal inferences {\bf Yet, we know that our estimates
  depend on their corresponding modeling assumptions and that
  different specifications can yield very different causal
  inferences}.
...
consistent with his or her prior expectations {\bf it is
  \emph{possible} for the researcher to find a specification
...
on matching. {\bf a new statistics literature on causal inference
  which is based on matching, a nonparametric and non-model-based
  statistical procedure} This work offers considerable promise for
increasing the reliability and validity of causal inferences by
reducing or eliminating the role of functional form and other
assumptions of parametric models.  The statistical literature on
...
literature and {\bf end the sentence here and start a new one: In
  particular, we suggest a procedure by which ...}  suggesting a
procedure by which applied researchers can make productive use of the
ideas developed without giving up most of their present analytical
strategies.  Our simple unifying idea is to use these new
nonparametric techniques not as substitutes for our present parametric
regression techniques, but to make our familiar parametric techniques
work better.  Under our inferential framework, analysts merely add a
simple preprocessing step to their data analysis procedures and then
employ whichever models they were previously accustomed to using to
analyze the preprocessed data rather than the raw data {\bf this
  sentence is a bit long and awkward}.  All of the intuition,
diagnostics, and knowledge about our parametric procedures can then be
used as before.

Parametric techniques applied to the resulting {\bf remove resulting}
preprocessed data are typically far less sensitive to the choices of
modeling assumptions researchers make {\bf remove researchers make}
than the raw data {\bf this part seems a bit awkward} and {\bf
  therefore } produce more valid causal inferences.  For readers,
