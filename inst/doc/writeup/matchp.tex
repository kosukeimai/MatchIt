...
assess balance will never be adequate (unless matching is exact).  

In addition, the ultimate goal of matching is not merely balance, but
...
\citet{DiaSek05} and which our software incorporates, can also be
useful in finding good matches to evaluate.
...
{\bf formal definition of our balance statistic.} One of our balance
statistics for a particular variable $X$, the Maximum distance of
quantile functions, is given by
$$\max_{0 \le \alpha \le 1} |F_{Xt}^{-1}(\alpha)-F^{-1}_{Xc}(\alpha)|$$
where $F_{Xt}$ and $F_{Xc}$ are the empirical cumulative probability
distribution functions of the variable $X$ for the matched treatment
and matched control groups, respectively. That is,
$F_{Xt}(x)=\frac{1}{n'_t} \sum_{i=1}^{n'} {\rm I}_{\{X_i \le x\}} t_i$
and $F_{Xc}(x)=\frac{1}{n'_c} \sum_{i=1}^{n'} {\rm I}_{\{X_i \le x\}}
(1-t_i)$ where $n'_t=\sum_{i=1}^{n'} t_i$ and $n'_c=\sum_{i=1}^{n'}
(1-t_i)$ are the size of matched treatment and matched control groups,
$n'=n'_t + n'_c$ is the size of matched data, and ${\rm I}$ denotes
the indicator function.

{\bf power function of two-sample $t$-test with equal variance.} the
statistic for two sample $t$-test is given by
$$ \frac{\sqrt{n'_t+n'_c-2} 
  (\overline{X}_t-\overline{X}_c)}{\sqrt{\left(\frac{1}{n'_t}
      +\frac{1}{n'_c}\right)S^2_X}}$$ where $S^2=\sum_{i=1}^n t_i(X_i
- \overline{X}_t)^2+ (1-t_i)(X_i - \overline{X}_c)^2$, which is
distributed as the $t$ distribution with $n'_t+n'_c-2$ degrees of
freedom. The power function is given by
$$ T_{n'_t+n'_c-1}(-c_{\alpha/2} \mid \phi) +
1-T_{n'_t+n'_c-1}(c_{\alpha/2} \mid \phi) = 2
T_{n'_t+n'_c-1}(-c_{\alpha/2} \mid \phi)$$ where $T_\nu$ is the
distribution function of $t$ distribution with $\nu$ degrees of
freedom, $c_{\alpha/2}=T_{n'_t+n'_c-1}(1-\alpha/2)$ and
$$\phi = \frac{\mu_1-\mu_2}{\sigma\sqrt{\frac{1}{n'_t}+\frac{1}{n'_c}}}$$
