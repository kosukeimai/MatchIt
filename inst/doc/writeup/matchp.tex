...
\begin{abstract}
  ...
  the ones to include from numerous trial runs readers never see.
  Given the typically large variation in estimates across choices of
  control variables, functional forms, and other modeling assumptions,
  how can researchers convince readers that the few estimates
  presented are accurate or representative?  How do readers know that
  political science articles are not merely demonstrations of the
  \emph{possibility} to find a specification that fits the author's
  favorite hypothesis?  Matching methods offer the promise of causal
  inference with fewer assumptions but the literature suffers from
  conflicting approaches to estimation, uncertainty, theoretical
  analysis, and practical advice.  In this paper, we propose a unified
  perspective on this literature by suggesting matching a method of
  nonparametric preprocessing for reducing model dependence in
  parametric methods.  This approach makes it possible for researchers
  to preprocess their data (such as with the easy-to-use matching
  software we offer) and then to apply whatever familiar techniques
  they would have used anyway.  Under our approach, instead of using
  matching to replace existing parametric methods, we use it to make
  existing methods work better, such as by giving more accurate and
...
only arises in private and so open discussions of it are only
infrequently found in the literature.

To see the problem, consider how political scientists typically
conduct quantitative research: Even for a single project, we often
spend a year or more collecting, correcting, recollecting, merging,
and recoding data.  When all the data are finally available, in the
right format, and loaded into our favorite statistical package, we
obtain a causal estimate by running some parametric statistical
procedure --- regression, logit, probit, duration models, structural
equation models, count models, etc.  This run typically takes only a
few seconds and, according to some textbook ways of proceeding, it
would be time to write up the results.
...
a forth with a different measure of our key causal variable, one with
...
again.  Although this usual procedure produces hundreds or even
thousands of alternative estimates of our causal effect, we typically
only choose one, and rarely more than say 5--10, to present in a
paper.  Yet, we know that the dependence of our estimates on these
modeling assumptions means that different specifications can produce
very different causal inferences.

The problem for researchers is how can to convince readers that we
picked a representative specification or the right one rather than the
one that most supported our favorite hypothesis?  What does it even
mean to choose ``the right'' parametric model estimates when any
...
that the researcher found it \emph{possible} to find a specification
consistent with his or her prior expectations?

We attack this problem by drawing from a new statistics literature on
nonparametric, non-model-based inference, that focuses on a diverse
collection of matching estimators for causal inference.  This work
...
functional form and other assumptions of parametric models.  The
statistical literature on matching has grown quite sophisticated, but,
from the point of view of the practical researcher, it looks like a
cacophony of conflicting techniques, practices, conventions, and rules
of thumb.  Valid methods for computing standard errors and confidence
...
literature and suggesting a procedure by which applied researchers can
make productive use of the ideas developed without giving up most of
their present analytical strategies.  Our simple unifying idea is to
use these new nonparametric techniques not as substitutes for our
...
procedures and then employ whichever models they were previously
...
Parametric techniques applied to the resulting preprocessed data are
typically far less sensitive to the choices of modeling assumptions
researchers make than the raw data and produce more valid causal
inferences.  For readers, preprocessing is thus good insurance that
the author did not unintentionally cook the books; for investigators,
it is a productive method for finding systematic patterns in our
typically noisy data without having to worry as much about whether
unverified modeling assumptions are correct.  Our preprocessing
approach has obvious advantages in terms of exposition, pedagogy, and
ease-of-use, since our methodology builds on rather than replaces what
we have already learned, brings some order and cohesion to conflicting
methodological literatures, and suggests a relatively straightforward
approach for empirical researchers seeking to make causal inferences.
For example, our approach suggests a simple, standardized, and valid
approach to computing standard errors and confidence intervals for
most of the new matching techniques.  This strategy also made it
possible for us to write easy-to-use software that implements all the
ideas we discuss in this paper; it is free and available at
...
% *** insert brief statement of results with se's.
