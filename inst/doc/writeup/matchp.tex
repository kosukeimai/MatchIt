...
have intuitively chosen similar approaches, such as in
\citet{Rosenbaum86, GlaLevMye03}.  It is also similar in spirit to
methods in other areas that preprocess data so that subsequent
analyses can be improved without modifying existing techniques, such
as multiple imputation \citep{Rubin87,KinHonJos01} and outlier and
feature detection \citep[][Ch.8]{Bishop95}.
...
reference to the pretreatment variables. {\bf A funny thing is that
  ``realized'' causal effect is never realized (or observed).}

A causal effect is a function of \emph{potential outcomes}: let
$y_i(t_i=1)\equiv y_i(1)$ {\bf this should be $y_i(1)\equiv
  y_i(t_i=1)$.} be the vote we would observe in district $i$ in say
the 2008 election if in fact the Democratic incumbent receives his or
her party's nomination (i.e., $t_i=1$) and $y_i(t_i=0)\equiv y_i(0)$
{\bf this should be $y_i(0)\equiv y_i(t_i=0)$.} the vote the party
would receive if the Democratic Party nominates a nonincumbent
($t_i=0$).  The use of parentheses in this notation denotes that the
outcome is potential, and so not necessarily observed, and that it
depends on the value of the variable in parentheses.  A key point is
that the values of these potential outcomes remain the same regardless
of whether the treatment is in fact applied in district $i$ or not.
However, since the Democratic Party either will nominate ($t_i=1$) or
will not nominate ($t_i=0$) an incumbent to run in district $i$, we
will observe either $y_{i}(1)$ or $y_{i}(0)$ but not both.\footnote{We
  assume throughout that every unit within each treatment group
  receives the same treatment.  This assumption would be violated in
  our running example if the nomination the Democratic party gives to
  the incumbent in some districts means something different from that
  in other districts.  We also assume that the treatment in one unit
  has no effect on the potential outcomes in any unit other than its
  own.  Together these two assumptions are sometimes known under the
  awkward name of the ``stable unit treatment value assumption'' or
  SUTVA \citep{Rubin74}.}
...
realization we see and could have led to some other realization. {\bf
  This sentence is a little unclear to me. We might want to make a
  clear distinction between in-sample (or finite population) and
  out-of-sample (or population) inferences. For example, the former is
  used to make inferences about a particular election at hand, whereas
  the latter attempts to make generalization about some underlying
  population. }  This logic then produces the random causal effect:
\begin{equation}
  \label{rance}
...
\begin{align}
  ...
  &= E[Y_i(1) - Y_i(0)]\\ \notag &= \mu_i(1) - \mu_i(0),
\end{align}
where $E[Y_i(1)]\equiv\mu_i(1)$ and $E[Y_i(0)]\equiv\mu_i(0)$. {\bf
  Again, RHS and LHS should be flipped.}
...
observations or some subset of observations. {\bf or for a particular
  (subset of) population} This leads to two choices for quantities of
interest, for either realized and random causal effects (we show them
here for the latter).  First is the population average treatment
effect or ATE:
\begin{align}
  \label{pate}
...
{\bf The second equation is an estimator of the first equation, but
  not the estimand. alternatively, the second equation can be
  interpreted as the estimand of the sample aerage treatment effect. }

The alternative choice of a target quantity of interest is the {\bf
  population} average treatment effect on the treated, or ATT:
\begin{align}
  \label{att}
...
interest {\bf this sentence seems a bit awkward.}.  In this paper, we
usually focus on ATT as the quantity of interest when it is
...
Although true {\bf classical randomized instead of ``true''}
experiments are only very rarely conducted in political science, they
remain a useful ideal type for understanding other types of research.
In addition, {\bf rather than using ``in addition'' should we use a
  different word because this is something important and needs to be
  emphasized?} some of the assumptions necessary for valid inference
in experiments will be approximated by the preprocessing procedures we
suggest below.

Valid and relatively automatic {\bf ``straightforward'' rather than
  ``automatic''?} causal inferences can be achieved via classical
randomized experiments (where by ``classical'' we refer to the
situation where the random assignment is complete and not applied
within categories of other variables {\bf ``not applied ...'' is not
  clear to me}).  Such experiments have three critical features: (1)
\emph{random selection} of units to be observed from a given
population, (2) \emph{random assignment} of values of the treatment
($t_i$) to each observed unit, and (3) a \emph{large $n$}.
...
$y_i$ conditional on $t_i$ {\bf or in other words, $X_i$ must be
  correlated with the potential outcomes, $y_i(1)$ and $y_i(0)$}.  If,
instead, one or more of the three conditions do not hold, then $X_i$
may be omitted without any bias resulting {\bf although variance may
  be larger}.  Random assignment guarantees that $t_i$ is unrelated to
...
{\bf Classical randomized} Experiments are a true ideal type,
particularly in relation to most social science research where almost
all research fails to meet at least one of the three features.  Even
most social science lab experiments have random assignment but no
random selection and often a small $n$.  Traditional survey research
has what is intended to be random selection (although with
dramatically increasing nonresponse rates and cell phone usage, this
is a less plausible claim) and certainly has a large $n$, but random
assignment, except when the treatment involves the wording of survey
questions, is usually impossible.
...
$y_i$ conditional on $t_i$ {\bf citation? we also need to be careful
  about whether to use $y_i$ or $Y_i$ and $T_i$ or $t_i$}.  In other
...
$X_i$, and so we can literally ignore all unobserved variables {\bf
  \citep{RosRub83}}.  Ignorability is a strong condition, but it is
one about which social scientists are deeply knowledgeable and which
is the central methodological concern of many substantive scholarly
...
Researchers in the situation of knowing the parametric model {\bf
  nobody knows the true model and so I would say ``researchers who are
  willing to assume a parametric model''} that generated their data,
up to but not including some unknown parameters, should specify and
directly estimate this parametric model.  Preprocessing with matching
procedures is suboptimal in this situation.  Of course, few
researchers with real observational data sets in the social sciences
have this kind of knowledge and as a result some choices need to be
made among the range of possible parametric models. {\bf I guess we
  say it here. but even with experimental data, we don't know the true
  parametric model.}
...
{\bf known rather than specified?}  probability density $p(\cdot)$,
...
+ t_i\beta + X_i\gamma)$ {\bf it should be $\mu_i \equiv E(Y_i \mid
  t_i, X_i) =...$}, for some specified functional form {\bf I would
  use ``known function''} $g(\cdot)$ and with intercept $\alpha$ and
...
{\bf remove the quotation marks and include McCulloch and Nelder
  citation} as well as many others.  For example, if $p(\cdot)$ is
normal and $g(c)=c$, we have linear regression; if $p(\cdot)$ is
...
\end{align}
{\bf again, it seems to me that $\mu_i(j)$ is defined here and so that
  should come as LHS.}  We can produce estimates of these quantities
by assuming independence over observations, and forming a likelihood
function
\begin{equation}
  \label{lik}
...
In experimental data, classical {\bf remove ``classical''} random
assignment guarantees that $T_i$ and (any observed or unobserved)
$X_i$ are independent {\bf the independence is between the potential
  outcomes and the treatment assignment as well as between the
  covariate and treatment assignment. the latter gurantees no omitted
  variable bias}.  Since stochastic independence implies mean
independence, we can drop $X$ and write
...
\end{align}
{\bf Is $\mu_i(j)$ defined with or without conditioning on $x_i$?}
...
\end{equation}
{\bf What about ATE? also should we make the distinction between
  population and in-sample inference? If it's the latter, we need to
  condition on the observed $y_i$ in the sample.}  which most
importantly no longer has a summation sign over $i$.
...
the expected potential outcomes.\footnote{To rule out degenerate cases
  ...
  {\bf range of the} potential outcomes be the same.} Moreover, given
any chosen stochastic component, this result holds for virtually any
{\bf replace ``virtually any'' with ``a wide range of'' or such}
parametric model, including all the special cases of the general model
given above.
...
{\bf maybe we should mention why one might want to do this in
  experimental data; (1) improve the efficiency (2) conducting
  conditional inferences}, the absence of model dependence means that
the choice for a fuxnctional form will not matter: The results will be
the same no matter what the choice for $g(\cdot)$ is.
...
multidimensional, this is a surprisingly difficult task.
...
{\bf equivalently} 10 mean indicator variables).  In contrast, the
usual approach to estimation is to assume linearity.  This enables us
to enter not 10 indicator variables, but rather only a constant term
and one slope coefficient.  How do we get from ten parameters to only
two?  Pure assumption.  If we have some sense that the relationship is
indeed linear or close to linear, this is a good use of external
information to reduce the number of parameters that must be estimated.
If not, then we still have the best {\it linear} approximation to the
...
\begin{figure}[t] 
 \begin{center}
  ...
  \caption{Model sensitivity of average treatment effect estimates for
    ...
    control units represented by ``C.'' The vertical axis plots the
    outcome of interest, while the horizontal axis plots some
    confounding covariate.  The panels depicts estimates of the
    average treatment effect for a linear and quadratic specification,
    represented by the difference between parallel lines and
    parabolas, respectively.  In the raw data, plotted in the left
    panel, the models extrapolate for treated units with $X$ values
    below 4 and are therefore sensitive to specification.  In the
    matched data, plotted in the right panel, treated units are
    matched with control units that are close in $X$ (grey units are
    discarded) and as a result treatment effect estimates are similar
    irrespective of model specification.}
  \label{fg:extrap}
\end{figure}

To illustrate model sensitivity to specification in a very simple
setting, Figure~\ref{fg:extrap} plots some artificial data with
outcome $Y$, confounding covariate $X$, and a treatment of interest.
The left panel plots the raw data with simple linear and quadratic
fitted values. The difference between the parallel lines or parabolas
represents the estimated treatment effect, which here appears to be
positive for the linear model and negative for the quadratic model.
No treated units exist, however, with $X$ values below 4, and the
models thereby extrapolate from the data.  Since we don't observe any
treated units with $X < 4$, the data provide little information to
estimate what the counterfactual outcome for control units with $X <
4$ would be.  Morever, these control units appear to lead to an
awkward fit of the models in the range of $X$ where our data is
actually informative.  Confining ourselves to the range of units which
are actually close in $X$ by matching, the details of which we discuss
next, linear and quadratic specifications yields virtually identical
treatment effect estimates in the right panel.  While astute
researchers may point out that residual diagnostics should uncover
misspecification in the left panel, the myriad of specification
choices, and concordantly the difficulty of diagnosing
misspecification, expands dramatically as the covariates grow.  {\bf
  Should we talk about extrapolation problem a little more more
  explicitly? Our claim seems to be that functional form assumption is
  less problematic when no extrapolation is made.}
...
(\cite{FraRub02}, \cite{Greenland03}, \citet{KinZen04}).
...
with little inefficiency: go to the parametric analysis.  If
...
\cite{RosRub83}.  The propensity score is the true probability of unit
$i$ receiving treatment, given the covariates $X_i$, $e_i(X_i) =
...
procedure balances $X$ (and procedures for checking balance are
offered in Step 4), then we use it.  If not, then we try respecifying
the logistic regression by adding interactions or squared terms and
...
usually reduces the variance somewhat less than the previous one and
will moderately increase bias).  If, instead, fewer controls are
available than those treated, then matching with replacement ---
allowing each control unit to be matched to more than one treated unit
--- is a good option.  Alternatively, consider switching the
definition of treatment and control groups (although, of course, if
using ATT, this will change the substantive definition of the causal
effect).
...
deviations of the propensity score computed from all other variables.
The Mahalanobis distance is the weighted average of the squared
distance between units $i$ and $j$, $(X_i-X_j)'\Sigma^{-1}(X_i-X_j)$,
where $\Sigma$ is the variance covariance matrix of $X$ in the control
group.  See \cite{RubTho00}.
...
different methods of presenting (\ref{balance}) in our two
applications.
...
of new drug approval approval times by the U.S.\ Food and Drug
Administration (FDA)\footnote{We focus specifically on Model 1 of
  Table 2 \citep[p. 499]{Carp02}.}.  Here, we focus on the hypothesis
that ``expected approval times are greater when Democrats control the
White House, when the agency's oversight committees are more liberal,
and when the House and Senate are more liberal''
\citep[p.495]{Carp02}.  In short, liberal FDA oversight should lead to
slower approval of new drugs.  This hypothesis is substantively
important to the literature on institutional and partisan determinants
...
In our analysis, we focus on the estimation of the marginal effect of
...
the treatment and control groups.  For example, drugs introduced under
...
and were submitted at times with fewer FDA staff.  However, after
matching, these differences vanish -- all t-statistics are less than
1.96 implying that the mean differences are no longer statistically
...
with parametric models applied to raw data is that it is these control
units which will be most influential in any model fit to the raw data
set.  By choosing the matched set of control units which look more
similar to the treated units, the treated-control comparison will then
take place only among units on the background variables and thus will
not be affected as much by the model specification.
\begin{figure}[t] 
 \begin{center}
...
(i.e., all possible subsets of covariates from the 18).  Even ignoring
possible interactions and nonlinearities, this amounts to $262,143$
survival analyses. Figure~\ref{fg:fdadens} presents the density plot
for the estimated average treatment effects for the raw data as well
as the matched (preprocessed) data.  The estimates are far more model
dependent using the raw data than using the matched data.  For
example, the variance of the estimated average treatment effect from
the matched data is more than three times as large as that from the
raw data.  The point estimate from the raw data with all covariates
implies that a Democratic Senate majority decreases the expected
approval time by about 30 months, whereas many specifications indicate
that the estimate is much less.  Using the raw data, Carpenter notes
the large model sensitivity, concluding that oversight covariates
appear not to matter.  However, the result from the matched data
indicate that the effect size may even be larger and less sensitive
than suggested by the raw data.
...
where high scores indicate greater conservatism).\footnote{Visibility
  ...
  the correctness of these measurements or methods.}  We confine
...
nonparametric estimation would require estimation of 44 million
parameters, and with a dataset with only 494 observations.  Koch
reduces these parameters to only six (main effects) by the assumptions
of no other interactions and nonlinearities.
...
results in 31 discarded units, and ensures that outliers do not
contaminate our data.  Next, we create six subclasses by quantiles of
...
are able to compared the most similar candidates within each subclass.
Indeed, across subclass-covariates the t-statistic exceeds 2 only
once, which is roughly the same as we would expect if treatment were
assigned randomly within each subclass.
\begin{figure}[t] 
 \begin{center}
 ...
 \caption{Density estimates of estimated effects of
   ...
   presents estimates for the full dataset and the solid line for the
   ...
   sensitive to model specification for the full dataset compared to
   the matched dataset.}
 \label{fg:kochdens}
\end{figure}
...
more liberal.  The point estimate from the full data that controls for
...
raw data.  The range of coefficient estimates is substantially
