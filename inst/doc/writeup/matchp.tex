...
are often conflicting, absent, or misunderstood.\footnote{Matching
  ...
  observational studies in some disciplines, such as epidemiology and medicine.  However, the
...
and \citealt[][pp.73--4]{Rosenbaum02}).  This is of course a critical
point, since controlling for the consequences of a causal variable can
severely bias a causal inference; the problem is unfortunately too
common \citep{KinZen06a}.
...
the ATE.  If causal effects are constant over $i$, then the ATT and the ATE
are identical.\footnote{To change these sample causal effects to
  ...
  probability of receiving treatment.  Point estimates for the two
...
within each pair of units.\footnote{Formally, ignorability (by which
  ...
  $p(Y(0)|t=1,X)=p(Y(0)|t=0,X)$ and $p(Y(1)|t=1,X)=p(Y(1)|t=0,X)$.}
...
pre-treatment explanatory variable available.
...
versa.\footnote{The two sample $t$-test statistic is given by $
  \sqrt{n_t+n_c-2}(\overline{X}_t-\overline{X}_c)/\sqrt{\left(\frac{1}{n_t}
      +\frac{1}{n_c}\right)S^2_X}$ where $S^2=\sum_{i=1}^n t_i(X_i -
  \overline{X}_t)^2+ (1-t_i)(X_i - \overline{X}_c)^2$.  The difference
  in means as a measure of balance is thus distorted in the t-test due
  to the number of remaining observations (the first factor in the
  numerator), the variance of $X$ in those observations (in the
  denominator), and the ratio of treatment to control observations.}
Using hypothesis statistics to assess balance in matching applications
should end.
\begin{figure}[t] 
 \begin{center}
...
of the treatment group against that of the control group in a square plot (we
...
line).\footnote{For example, the maximum distance of quantile functions, is
  given by $\max_{0 < \alpha < 1}
  |F_{Xt}^{-1}(\alpha)-F^{-1}_{Xc}(\alpha)|$ where $F_{Xt}$ and
  $F_{Xc}$ are the empirical cumulative probability distribution
  functions of a single variable $X$ for the matched treatment and
  matched control groups, respectively. That is,
  $F_{Xt}(x)=\frac{1}{n'_t} \sum_{i=1}^{n'} {\rm I}_{\{X_i \le x\}}
  t_i$ and $F_{Xc}(x)=\frac{1}{n'_c} \sum_{i=1}^{n'} {\rm I}_{\{X_i
    \le x\}} (1-t_i)$ where $n'_t=\sum_{i=1}^{n'} t_i$ and
  $n'_c=\sum_{i=1}^{n'} (1-t_i)$ are the size of matched treatment and
  matched control groups, $n'=n'_t + n'_c$ is the size of matched
  data, and ${\rm I}$ denotes the indicator function.}
...
run the parametric procedure with each unit weighted proportional to the number of
...
equation~\ref{att}.  Carpenter's remaining six oversight variables are
...
by \citet{KinZen06a}. The result indicates that none of the treated
and control units is in the convex hull. This lack of common support
explains in part why \citet{Carpenter02} finds the results are highly
model-dependent. To proceed, we estimate the propensity score using
logistic regression with all covariates as linear predictors.  We also
find that matching on the square root of hospitalization associated
with indication improves the resulting balance significantly, and so
we include it.  We then discard 15 control units and 2 treated units
that are outside of the common support of the estimated propensity
score. Finally, we conduct one-to-one nearest neighbor propensity
score matching (without replacement) while placing exact restrictions
on six binary variables (whether primary indication is lethal
condition, acute condition, and/or results in hospitalization; whether
disease mainly affects men, women, and/or children). We choose
one-to-one matching rather than one-to-many matching or full matching,
which keep more observations after matching, because our convex hull
analysis indicates that most of the observations are likely to be
outside of the common support.  This preprocessing procedure discards
the total of 102 units (10 treated units and 92 control units) from
the original sample that otherwise require substantial,
model-dependent extrapolations. The matched data sets then consists of
306 observations.
...
{\bf Clinical/Epidemiological Factors} & \\
...
{\bf Disease Politics (Groups and Media)} & \\
...
\end{tabular}
\end{center}
\caption{Covariate balance after matching for the FDA data. 
    The table presents three different measures of resulting balance
    after matching -- sample mean
    differences, and mean and maximum values of differences in
    empirical quantile functions -- as well as their percent balance
    improvement for each covariate of Model 1 of
    \citet{Carpenter02}. In almost all dimensions and across three
    different measures, matching significantly improves balance.  
}
...
to the preprocessed data, the estimated average treatment effect for
the treated based on Carpenter's Model 1 is approximately $-33.1$
months with an estimated standard error $8.8$, indicating that a
Democratic Senate majority significantly decreases the average
...
\begin{figure}[t] 
 \begin{center}
  ...
  \caption{Kernel density plot (a smoothed histogram)
    of the estimated insample average treatment effect of the
    Democratic Senate majority for the treated on FDA drug approval
    time across $262,143$ specifications. The solid line presents a
    density plot of the estimated insample average treatment effect
    for the treated (ATT) using the matched dataset, while the dashed
    line is based on the raw data.  The vertical arrow shows the point
    estimate from Carpenter's Model 1 based on the raw data.  The
    figure shows that ATT estimates are considerably more sensitive to
    model specification using the raw data as compared with the
...
Figure~\ref{fg:fdadens} presents a histogram for the estimated
(in-sample) average treatment effects for the treated across all the
specifications for the raw data as well as for the matched
...
For example, the variance of the estimated average treatment effect
from the matched data (the solid curve) is less than one tenth the
size of that from the raw data (the dashed curve).  The distribution
of estimates for the matched data is also closer to the normal
density, which will happen when control variables included are having
effects only due to random errors.
