...
to be equivalent.  Thus, to be clear, \emph{matching does not require
  pairing observations}; only the \emph{distributions} need be matched
as closely as possible.  Moreover, exact matching has the disadvantage
in many data sets of not generating many matches.  The problem is most
...
Many in the literature, modern econometricians most commonly, match
and then set as their goal producing variance estimators by making as
few assumptions as possible.  The variance estimates are from point
estimators based on a simple difference in means, or difference in
means with bias adjustment, following matching.  This nonparametric
strategy that tends to produce accurate, but often complicated and
application-specific, variance estimators \citep[e.g.,][]{AbaImb05}.

In contrast, our perspective (which is similar to the special cases
analyzed by some statisticians) is to begin with what social
scientists are now doing, which is estimating some form of parametric
(usually regression-type) model.  We then ask how matching as
preprocessing can improve this current practice.  Thus, since social
scientists are already making parameteric assumptions, we do not ask
anyone to change their assumptions and we do not reduce them either;
we merely ask how to compute variance estimate in social scientists'
current parameteric models if the data are also preprocessed.  The
answer turns out to be simple and the estimator highly convenient: use
the same variance estimator as one would normally do in using a
parametric analysis.
...
\begin{tabular}{r<{  }@{}r@{}c@{}l@{}c@{}c@{}>{  }l} 
  ...
  && $y_i$       &&       $1-y_i$  && \\
\end{tabular}
\caption{\em Notation for Precinct $i$.  The goal is to estimate the
