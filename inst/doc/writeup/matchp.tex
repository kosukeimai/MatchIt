...
\begin{figure}[t] 
 \begin{center}
  ...
  \caption{Dangers in relying on t-statistics as measure of balance.
    Each panel shows the effect of changing sample size on a measure
    of balance.  The solid lines indicate the average value of the
    statistic when a given number of control units are randomly
    dropped from the data set (out of a total of 245 control units).
    The dashed lines indicate the corresponding 95\% confidence
    intervals.  With larger numbers of control units dropped (i.e.,
    smaller numbers of control units in the resulting sample), the
    t-statistic gets closer to 0, falsely indicating reductions in
    balance with the smaller resulting sample sizes.  In fact, because
    controls are dropped randomly, the balance is the same across all
    data sets, regardless of sample size.}
  \label{fg:tstat}
\end{figure}
