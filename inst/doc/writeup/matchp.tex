...
\begin{figure}[t] 
 \begin{center}
  ...
  \caption{Model sensitivity of average treatment effect estimates for
    ...
    units represented by ``C,'' where the y-axis plots the outcome of
    ...
    between the parallel lines and parabolas, respectively.  In the raw
...
setting, Figure~\ref{fg:extrap} plots some simulated data with an
...
next, linear and quadratic specifications yield virtually identical
...
misspecification, expands dramatically as the number of covariates grows.
...
of new drug approval times by the U.S.\ Food and Drug
...
related to, and in part consequences of, the Democratic Senate majority
variable.)
...
and received less FDA staff time.  However, after matching, these
t-statistics are all less than 1.96, implying that the mean
differences are no longer statistically significant at the 5\% level.
The mean differences of the other covariates are small both before and
after preprocessing.\footnote{Preprocessing increases the values of
  the t-statistics for two variables. Uniform improvement of balance
  for all covariates is unlikely unless exact matching is possible
  and/or the sample size is large. The minor sample differences that
  remain, however, will be adjusted by fitting the parametric models
