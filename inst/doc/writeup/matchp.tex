...
The problem is that the parametric model almost universally chosen
after matching has been a simple difference in means without any
controls for potential confounding variables.  We simply point out
that, except in the extraordinary case where matching is exact, better
parametric procedures have the potential to greatly improve causal
inferences even after matching.
...
recommend matching followed by a different specific form of parametric
adjustment, as well as intuitively chosen strategies used in applied
research by \citet{Rosenbaum86} and others, as discussed in
...
causal effect,'' which is the simplest definition available, in that
it is the closest to the data.  We then generalize the definition to
include features of random causal effects that are useful for
understanding connections between nonparametric preprocessing and
parametric models, define causal effects of interest at the population
level, and discuss multiple and nonbinary treatments.   Appendix
\ref{s:connect} discusses connections between causal inference and the
literatures on missing data and ecological inference.
...
working hard to avoid post-treatment bias in the process.  We stick a
single binary treatment here, since it greatly simplifies the
...
treatment to each observed unit,\footnote{We assume below a classical
  ...
  assignment depends on observed values of the covariates, those
...
population is related to the dependent variable only by random chance.
...
related to the dependent variable $y$ after taking into account the
treatment $t$ and pretreatment covariates, $X$.  Whether this
...
graph of Figure~\ref{fg:extrap} plots artificial data for outcome $Y$
on the vertical axis and a pretreatment covariate $X$ on the
horizontal axis.  This data set was designed to best illustrate the
problem, not to be realistic; in real examples, aspects of the problem
we portray here often appear, but they may be more difficult to see
given the simultaneous presence of other methodological problems.  In
this figure, each data point is plotted as a ``T'' for treated units
($t_i=1$) and ``C'' for control units ($t_i=0$).  We then fit two
regressions to these data.  The first is a linear regression of $Y$ on
a constant, $t$, and $X$: $E(Y_i|X_i,t_i)=\alpha + t_i\beta +
...
counterfactual $x$ of interest.''  In practice, of course, additional
model dependence will occur when models are chosen that do not fit the
data.  The left graph in Figure \ref{fg:extrap} obviously combines
both sources of model dependence.
...
rather than a population density.\footnote{To be more specific, the
  ...
  $\sum_{i=1}^n (1-t_i)$ on the right.}  The simplest (although not
necessarily the best) way to understand how we can satisfy
(\ref{balance}) by preprocessing is via \emph{one-to-one exact
...
to be equivalent.  Thus, to be clear, \emph{matching does not require
  pairing observations} (indeed, there might have been less confusion
if the technique had been called ``pruning''); only the
\emph{distributions} need be matched as closely as possible.
Moreover, exact matching has the disadvantage in many data sets of not
...
and as a result increase mean squared error.  If this occurs, common
practice is to sacrifice some bias reduction for the increased
efficiency that comes from having more observations in the
preprocessed dataset.  In our approach, if we lose some opportunity
for bias reduction we do so only in the preprocessing stage; our
second stage parametric analysis still has a chance to eliminate the
remaining bias.  This second chance does not apply if only the
difference in means is used.  We therefore turn in
...
ruling out extreme cases where all data are dropped during matching
...
efficiency of estimates even while reducing bias.  This may seem like
a strange situation, and would seem to violate a first principle of
statistics, informally described as ``more data are better''.
However, more data are in fact better only when using an estimator
that is ``self-efficient'' \citet{MenRom03}, which roughly speaking is
...
N(\beta_0+\beta_1x_i,\sigma_i^2)$, for $i=1,\dots,1000$, and variance
\begin{equation*}
  \sigma_i^2 =
  \begin{cases}
    \theta & \text{for $x\leq 5$}\\
    500\theta & \text{for $x>5$}
  \end{cases}
\end{equation*}
where $\theta>0$ and say 50 observations reside at $x>5$.  With data
...
observations are dropped.  This suggests that so long as one keeps as
many observations as possible, matching will often reduce bias and
improve the efficiency of subsequent parametric analyses.
...
the treatment and control $X$ distributions resemble each other as in
(\ref{balance}), without losing too many observations in the process.
The main diagnostic of success is also balance, the specific details
of which we describe below, as well as the number of observations
remaining after matching.  Trying different matching methods is not
like trying different models, some of which are right and some wrong,
since balance provides a straightforward objective function to
maximize and choose matching solutions.  Just as we iterate a
numerical likelihood function to its optimum parameter values (and
ignore any intermediate parameter values on the way to the MLEs), one
should try as many matching solutions as possible and choose the
\emph{one} with the best balance as the final preprocessed data set.
Although this point is often misunderstood (such as by occasional
mistaken claims in the literature that differences across matching
solutions should contribute to uncertainty estimates), matching
solutions with suboptimal balance are in fact irrelevant and should
play no part in our ultimate inferences.

To ensure that selection during preprocessing depends only on $X$ (to
prevent inducing bias), the outcome variable $y$ should not be
...
treatment group.  With one pre-treatment covariate, the convex hull of
...
units with $X$ greater than max($X$) or less than min($X$) are
...
involves checking whether (\ref{balance}), $\tilde p(X|t=1)=\tilde
p(X|t=0)$, holds.  One way to think about this process is to imagine,
for all the variables in $X$, forming a multidimensional histogram for
all the treated units and comparing it to another multidimensional
histogram of all the control units.  Because of the curse of
dimensionality, multidimensional histograms with more than a few
covariates tend to be very coarse or have many empty bins, and so are
difficult to evaluate and compare.  Thus researchers usually examine
various low dimensional summaries instead.  If a low dimensional
summary differs between the treated and control groups then we know
(\ref{balance}) does not hold.  The risk of course is that even if the
treatment and control groups match according to some low dimensional
summaries, we still cannot be certain that (\ref{balance}) holds since
it is a multivariate concept, and so using several different checks is
always a good idea.
...
means (or the equivalent of a regression of $Y$ on $T$ without any
control variables).  This is unfortunate, since the procedure assumes
that $T$ and $X$ are unrelated.  If the assumption is false, and it is
false except in the rare case when exact matching is possible for all
observations, then the result is the same for omitted variable bias
that occurs whenever a potential confounding variable is ignored.
...
best practice at statistical modeling were not followed, such as
forgetting to focus on a single causal variable at a time, not
avoiding post-treatment bias, etc.\footnote{The one exception that
  would suggest a change from standard parametric data analysis
  procedures is that, for subclassification, the parametric analysis
  ...
  number of units in each subclass), or, if insufficient observations
  exist within each subclass, fit an overall model with fixed or
  random effects for the subclasses \citep{ImaDyk04}.}  Using
preprocessed data should reduce model dependence, and this too is
worth checking: as one should even without preprocessing, we should
check the sensitivity of causal effect estimates to changes in the
specification.
...
Some in the literature match and then set as their goal producing
variance estimators by making as few assumptions as possible.  The
variance estimates are from point estimators based on a simple
difference in means, or difference in means with bias adjustment,
following matching.  This nonparametric strategy that tends to produce
accurate, but often complicated and application-specific, variance
estimators \citep[e.g.,][]{AbaImb05}.
...
matching as preprocessing can improve this current practice.  Thus,
since social scientists are already making parameteric assumptions, we
do not ask anyone to change their assumptions and we do not reduce
them either; we merely ask how to compute variance estimate in social
scientists' current parameteric models if the data are also
preprocessed.  The answer turns out to be simple and the estimator
highly convenient: use the same variance estimator as one would
normally do in using a parametric analysis.
...
variables $X$ (and $t$), which are therefore treated as fixed and
...
$\mu_i(1)$ and $\mu_i(0)$ and their uncertainty as usual from a
...
the estimated propensity score. Finally, we conduct one-to-one nearest
neighbor propensity score matching (without replacement) while placing
exact restrictions on the six binary variables (whether primary
indication is lethal condition, acute condition, and/or results in
...
feasible, or likely to reduce bias and model dependence, and apply
this criterion.
