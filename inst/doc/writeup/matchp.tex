...
% - extrapolation graph
% - redefine qoi if kill overlap: if ate or att and drop treated, but
%   not otherwise
% - no selection bias if qoi is in-sample (which is solution by definition)
% - edit liz (pscore) and dan's sections
...
for Improving Parametric Causal Inference}
...
of the new techniques.  Our approach is similar in spirit to
\citet{AbaImb04,*****}, who recommend (specific forms of) parametric
adjustment following matching, as well as \citet{HecTo***} who develop
forms of matching that are combined with semi-parametric (kernal
weighting) analyses.  It is also similar in spirit to methods in other
areas that preprocess data so that subsequent analyses can be improved
without modififying existing techniques, such as multiple imputation
\citep{Rubin**,KinHonJos00} and outlier and feature detection
\citep{cite from king/zeng paper}.  Our approach has also made it
...
procedures, see \citep{Angrist**,Imbens04,Rosenbaum02,Stuart04} and
the detailed user's guide to the software that accopmanies this paper
...
not losing too many observations in the process.  The main diagonstic
...
be tried, wih the final preprocessed data set the one with the best
...
\paragraph{1.  Covariate Selection}  
...
with matching than in parametric techinques, including variables only
...
\citep{RubTho96, Heckman98a}.  To avoid posttreatment bias, be sure to
...
\paragraph{3. Propensity score matching}  
...
\cite{RosRub83a}.  The propensity score is the true probability of
...
of motiviating the goal of better propensity score specification, and
...
raw covariates.  Of course, wnce we have balance on the covariates,
...
score.  The goal of matching is to acheive the best balance for a
...
The collective wisdom of the literature also recommends three more
...
variables severly bias of our causal effect.  To avoid this problem,
we match using two separate metrics, one of the large-effect variables
and another for the rest.  Thus, if feasiable, we create pools of
...
Finally, if some control units fall outside the range of the treated
...
\paragraph{4. Assess the matching procedure}
Use a propensity score specification procedure such as that described
in \cite{DehWah02} or the MatchIt manual.  Then use diagnostic plots
and summary statistics to examine how well the matching worked.
  \begin{enumerate} 
