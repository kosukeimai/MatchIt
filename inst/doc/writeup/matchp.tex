...
purpose R package for presenting and interpreting results called Zelig
\citep{ImaKinLau04}.
...
\citet{Hansen04} can be utilized.  That is, if balance is achieved
with a reasonable number of observations within each subclass, then
the analysis procedure is carried out within each subclass and the
results are averaged, or else an overall model can be run with fixed
or random effects for each subclass.
...
respective $X$ variable, then better balance is needed (\cite{Cochran68}).  We can also
