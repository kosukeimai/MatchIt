...
#setwd("c:/R/match/docs/writeup/figs/")
...
legend(2, 7, lty=c(1, 1, 2, 2), col=c(1, 8, 1, 8), lwd=1.5,
...
abline(lm.m1$coef[1] + lm.m1$coef[2], lm.m1$coef[3], lty=1, lwd=1.5)
abline(lm.m1$coef[1], lm.m1$coef[3], lty=1, col="darkgrey", lwd=1.5)
...
lines(plot.pts, lm.m2$coef[1] + lm.m2$coef[3]*plot.pts +
      lm.m2$coef[4]*plot.pts^2, lty=2, col="darkgrey", lwd=1.5)
legend(2, 7, lty=c(1, 1, 2, 2), col=c(1, 8, 1, 8), lwd=1.5,
       legend=c("Linear Model, Treated Group",
         "Linear Model, Control Group",
         "Quadratic Model, Treated Group",
