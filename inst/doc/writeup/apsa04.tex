...
\item Example: estimating the electoral advantage of incumbency for
  Democrats\pause
  \begin{itemize}
  \item Unit: congressional districts, $i=1,\dots,n$.\pause
...
\begin{itemize}
\item Classical randomized experiments:\pause
  \begin{enumerate}
  \item random selection of units from a give population.\pause
  \item random assignment of values of the treatment.\pause
  \item a large $n$.\pause
  \end{enumerate}
\item Observational studies:\pause
  \begin{itemize}
  \item do not meet all three features of randomized experiments\pause
  \item almost all political science research
  \end{itemize}
...
\item Assumptions for causal inference:\pause
  ...
  \item no interference among units.\pause
  \item the same treatment.\pause
  \end{itemize}
\end{itemize}
...
\item Preprocessed data set\pause
  ...
  \item $p(X\mid t_i=1) = p(X\mid t_i=0)$ for one-to-one
    matching.\pause 
...
\item Select Covariates: include all variables that would have been
  included in the parametric model. avoid posttreatment bias.\pause
