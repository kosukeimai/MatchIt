...
m1 <- matchit(mfml, data=dta.full, method="subclass", subclass=6, discard="both")
dta.match <- match.data(m1)
summary(m1)

pdf("KochQQSub1.pdf")
plot(m1, interactive=FALSE, which.xs=c("repcan1", "goppty"), subclass=1)
dev.off()

pdf("KochQQSub13.pdf")
plot(m1, interactive=FALSE, which.xs=c("repcan1", "goppty"), subclass=3)
dev.off()
