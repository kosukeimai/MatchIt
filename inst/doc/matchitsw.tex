...
by typing at the R command prompt, %** can we do this automatically if
its needed, like we do in zelig?  <<results=hide,eval=FALSE>>=
install.packages("optmatch", contriburl =
"http://www.stat.lsa.umich.edu/~bbh/optmatch") @ For more information
...
Optimal matching is performed with \MatchIt\ by setting \texttt{method
  ...
  the R command prompt, %** can we do this automatically if its
  needed, like we do in zelig?  <<results=hide,eval=FALSE>>=
  install.packages("optmatch", contriburl =
  "http://www.stat.lsa.umich.edu/~bbh/optmatch") @ For more
