...
\item \texttt{formula}: formula used to calculate the distance measure for matching.
  It takes the usual syntax of R formulas, {\tt treat
...
\item \texttt{distance}: the method used to estimate the
  distance measure (default=logistic regression, {\tt logit}).
...
\item \texttt{discard}: whether to discard units that fall
  ...
  procedure (default=\texttt{none}).  Note that discarding units may change the quantity of
...
\item \texttt{reestimate}: whether the model for distance
  measure should be re-estimated after units are discarded (default=\texttt{FALSE}). The input
...
\item \texttt{verbose}: whether or not to print out comments
  indicating the status of the matching (default=\texttt{FALSE}). The input must be a logical
  value. 
\end{enumerate}
...
\begin{enumerate}
\item \texttt{sub.by}: by what criteria to subclassify (default=\texttt{``treat"}).
  \texttt{"treat"} indicates by the number of treatment units, \texttt{"control"} indicates by the number of control
...
\item \texttt{ratio}: the number of control units to match to
  ...
  \texttt{match.matrix}.
\item \texttt{exact}: variables on which to perform exact
  matching within the nearest neighbor matching (default=NULL).  If \texttt{exact} is
...
\item \texttt{caliper}: the number of standard deviations of
  ...
  \end{itemize}
\item \texttt{subclass} and \texttt{sub.by}.  See the options for subclassification
   for more details on these options.  If a
...
\item {\tt ...}: additional inputs that can be passed to the
...
%\subsection{See Also}
%The complete manual of {\tt MatchIt} is available at
%\href{http://gking.harvard.edu/matchit}{http://gking.harvard.edu/matchit}
