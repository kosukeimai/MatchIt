...
\item {\tt ratio} specifies the number of control units to be matched
  to each treatment unit, the default is {\tt ratio = 1}. 
\item {\tt ...} represents additional minor inputs that can be passed to the
...
\begin{enumerate}
\item Nearest neighbor matching using the estimated propensity score
...
@
\item Subclassification with four subclasses
<<>>=
