...
substantially more robust and less sensitivity to modeling
assumptions.  Matched data sets created by \MatchIt\ can also be
entered easily in \hlink{Zelig}{http://gking.harvard.edu/zelig/} for
...
propensity score is the probability that a treatment was assigned to a
unit given information in the control variables.  To propensity score
...
A \emph{caliper} is a value of the propensity score demoninated in
...
caliper size or match the nearest neighbor only when no units within
...
tune propensity score matching, often by chosing a subset of
...
outcomes, $Y_{1i}$ as follows.  First, we fit a model using the
...
range of other models as well.

First run matchit and then the chosen analysis model (linear
regression) using zelig:
\begin{verbatim}
> library(Zelig)
> match.out1 <- matchit(treat ~ age + educ + black + hispan + nodegree +
                      married + re74 + re75, data = lalonde)
...
be used first to fill in (``impute'') the missing values (both
covariates and outcomes), or the analysis should be done using only
complete cases (which we do not in general recommend).  For imputation
software, see Amelia at
(\href{http://gking.harvard.edu/stats.shtml}{http://gking.harvard.edu/stats.shtml})
or other programs at
\href{www.multiple-imputation.com}{www.multiple-imputation.com}.  For
