...
matching method in \MatchIt\ is what is known as ``greedy" matching, where the closest control match for each treated unit
is chosen  
one at a time, without trying to minimize a global distance measure.  
Another method, known as ``optimal" matching, finds the matched samples with the smallest average absolute
...
Assignment model specification:
matchit(formula = treat ~ age + educ + black + hispan + married + nodegree + re74 + re75, data = lalonde, full = T)
...
since many subclasses will have only one treated or control unit it 
...
the covariate of interest (e.g., the propensity score, age, or education), and the treatment indicator and the subclass
...
all units are utilized.  Finally, the t-statistc is the t-statistic of the coefficient of the treatment indicator in
the linear model predicting the covarite; this t-statistic reflects the differences between the treated and control groups,
...
from that described here; see Section \ref{fullmatch} for details}:
  \begin{itemize}
