...
\item \texttt{discard} specifies whether to discard units that fall
  ...
  \begin{itemize}
  \item \texttt{none} (default) discards no unit.  Use this option
...
\item \texttt{reestimate} specifies whether the model for distance
  ...
  default is \texttt{FALSE}.  This may be desirable for efficiency
...
\item \texttt{match.matrix} is an $n_1$ by \texttt{ratio} matrix
 ...
 \item \texttt{distance} is a vector of estimated distance measure.
 ...
 \item \texttt{subclass} contains the subclass index in an ordinal
   ...
   and optimal matching).  Unmatched units have \texttt{psclass=NA}.
