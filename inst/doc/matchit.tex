...
\item \texttt{distance}, specifies the method used to estimate the
  distance measure. The default is the logistic regression, {\tt
    logit}. Currently, the following methods are available:
  \begin{enumerate}
...
log-log link), {\tt linear.cloglog} (complementary log-log link with
linear propensity score), {\tt log} (log link), {\tt linear.log} (log
link with linear propensity score), {\tt cauchit} (cauchy CDF link),
{\tt linear.cauchit} (cauchy CDF link with linear propensity score).
...
(complementary log-log link), {\tt GAMlinear.cloglog} (complementary
log-log link with linear propensity score), {\tt GAMlog} (log link),
{\tt GAMlinear.log} (log link with linear propensity score), {\tt
  GAMcauchit} (cauchy CDF link), {\tt GAMlinear.cauchit} (cauchy CDF link
with linear propensity score).

\item \texttt{nnet}, neural network model ({\tt nnet()} in the {\tt
    nnet} package).
  
\item \texttt{rpart}, classification tree ({\tt rpart()} in the
  \texttt{rpart} package).
\end{enumerate}
