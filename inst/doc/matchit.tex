...
\item \texttt{formula} takes the form of {\tt T \~\ X1 + X2}, where
  {\tt T} is a binary treatment indicator and {\tt X1} and {\tt X2}
  are the pre-treatment covariates, and {\tt T}, {\tt X1}, and {\tt
    X2} are contained in the same data frame.  The $+$ symbol means
  ``inclusion'' not ``addition.'' You may also include interaction
  terms in the form of {\tt X1:X2} or squared terms in the form of
  {\tt I(X1 \^\ 2)}.  The \texttt{I()} command ensures that the
  expression is interpreted as one term.  

  
\item \texttt{data} specifies the data frame containing the variables
  called in the formula.  You may find it helpful for the diagnostics
  to specify observation names in the data frame.  To do this, see
  Section~\ref{rnames}.  
...
\item{distance.options} specifies the optinal arguments that are
...
\item \texttt{discard} specifies whether to discard units that fall
  ...
  \item \texttt{none} (default) discards no units.  Use this option
    ...
    \citealt{Imai03}), when caliper matching will restrict the donor
...
they are returned as \texttt{NA} [EAS: This isn't quite true--they just aren't in there for now.
E.g., for method=nearest, the subclassification outputs aren't included at all in the matchit object. 
...
\item \texttt{match.matrix} is an $n_1$ by \texttt{ratio} matrix
 ...
 \item \texttt{discarded} is a vector of length $n$ that displays
   ...
   discarded, and \texttt{=FALSE} otherwise. 
   
 \item \texttt{distance} is a vector of length $n$ with the estimated distance measure for each unit.
 ...
 \item \texttt{weights} is a vector of length $n$ that provides the
   ...
   number of treatment units to which it was matched, and the sum of the control weights is
 ...
 \item \texttt{subclass} contains the subclass index in an ordinal
   ...
   \texttt{subclass} (or the total number of subclasses from full
   or exact matching).  Unmatched units have \texttt{psclass=NA}.
 ...
 \item \texttt{model} stores the output of the model used to estimate
   the distance measure.  \texttt{summary(m.out)} will give the
...
As the literature on matching techniques is rapidly evolving, \MatchIt\ will strive to incorporate new developments. \MatchIt\ 
is thereby an evolving program.  Users may be concerned that analysis written in a particular version may not be compatible 
with newer versions of the program.  The primary way to ensure that replication archives remain valid is to record the version 
of \MatchIt\ that was used in the analysis.  Our website maintains binaries of all public release versions, so that 
researchers can replicate results exactly with the appropriate version (for unix-based platforms, see 
\hlink{http://gking.harvard.edu/src/contrib/}{http://gking.harvard.edu/src/contrib/}; for windows, see 
\hlink{http://gking.harvard.edu/bin/windows/contrib/}{http://gking.harvard.edu/bin/windows/contrib/}).

In addition, users may find it helpful to install packages with version control, using the {\tt installWithVers} command with 
{\tt install.packages}.  So for example, in the windows R console, users may download the appropriate version from our 
website and install the package with version control by:
...
R CMD INSTALL similarly permits users to specify this version using the {\tt --with-package-versions} option.  After having 
specified version control, different versions of the program may be called as necessary.  Similar advice may also be 
appropriate for version control for R more generally. 
