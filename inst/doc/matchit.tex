...
The {\tt neyman()} function can also be used with exact matching on covariates.  For exact matching, an overall estimated
treatment effect is calculated, using the {\tt psweights} as weights.
...
[This paragraph is really bad, I know.] Zelig is an easy-to-use R program that enables estimation of a large variety of 
statistical models, easily interpretable quantities of interest, and automated graphics and summaries.  More details,
and the package itself, are available at {\url gking.harvard.edu/zelig}. \MatchIt\ and Zelig can be easily used
together to enable estimation of causal effects in very general settings.

Here we provide a few examples of how to use Zelig with \MatchIt\ objects.   The general framework is that of imputing
the missing potential outcomes under control $Y(0)$ for the units in the treated group.  This is done by estimating a
model for $Y(0)$ using the matched control units, and then imputing $Y(0)$ for the matched treated units. Once
those potential outcomes are imputed, calculation of individual $i$'s average treatment effect is straightforward:
$Y_i(1)-Y_i(0)$.  The average treatment effect across the treated individuals can then also be easily calculated.

\subsubsection{Basic analysis with Zelig}
A few steps are required to obtain estimates of the average treatment effect:

First, using Zelig, fit a model of the outcome with the covariates or propensity score as predictors, using
just the matched control group.  Here we use a linear model ({\tt "lm"}), however Zelig can support a wide range of models.
For more details, see the Zelig documentation at {\url gking.harvard.edu/zelig}.
\begin{verbatim}
> library(Zelig)
> match.out1 <- matchit(treat ~ age + educ + black + hispan + nodegree +
                      married + re74 + re75, data = lalonde)
...
Finally, predict {\tt re78} for the $x$'s in {\tt x.out1}, using the model fit in the control group (output in {\tt 
z.out1}).
