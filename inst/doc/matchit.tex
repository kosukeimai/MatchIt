...
\item \texttt{sum.all} is a data frame which contains variable names
  ...
  \item pooled standard deviations for all covariates $X$, where
    \texttt{SD}$= s_X = \sqrt{\frac{1}{n-1} \sum_{i} (X_i -
      \overline{X})^2}$,
  \item summary statistics from a QQ plot [EAS: Dan, can you fill in here?]
...
\item \texttt{reduction} shows the percent bias reduction achieved in each of the
        balance measures in \texttt{sum.all} and \texttt{sum.matched}, defined as $100*\frac{a-b}{a}$, where
        $a$ was the balance statistic before matching and $b$ is the balance statistic after matching.
...
\item Jitter plots of the propensity score for treated and control
  units.
\item QQ plots of each variable [EAS: Dan, can you fill this in?]
\end{enumerate}
...
\textbf{Exact Matching.}  \texttt{exact} specifies whether exact
matching should be done.  \texttt{FALSE} (default) indicates no exact
matching.  \texttt{TRUE} indicates exact matching on all variables in
the assignment model of the \texttt{formula} (i.e., the propensity
score will never be generated).  A vector of variable names indicates
separate variables on which to exact match, in combination with
matching on the propensity score.  This option will not be different
from \texttt{exact=TRUE}, unless the list differs from the variables
in the formula.  (For example, \texttt{matchit(T \~\ X1 + X2, data,
  exact=TRUE)} is equivalent to \texttt{matchit(T \~\ X1, data,
  exact=cbind(X1,X2))}.  But \texttt{matchit(T \~\ X1 + X2, data,
  exact=X1)} will match units exactly only on \texttt{X1} and perform
nearest neighbor matching on the propensity score which uses
\texttt{X1} and \texttt{X2}.)  Variables should be entered as a vector
of names \texttt{exact=c("X1","X2")} that refer to variable names in
\texttt{data}.
...
\textbf{Subclassification} Subclassification options allow the
...
\subsubsection{Options}
\textbf{Full.}  \texttt{full} specifies full matching, which forms
subclasses using all treated and all control units.  When
\texttt{full=TRUE}, \texttt{nearest} is automatically set to
\texttt{FALSE}.  The default is to not do full matching
(\texttt{full=FALSE}).
  
\textbf{Full.Options().}  \texttt{full.options()} is an optional list
of additional options for the call to \texttt{optmatch} when doing
full matching.  See the \texttt{optmatch} documentation for more
information.
...
\textbf{Replacement matching.} \texttt{replace} is a logical vector
which specifies whether to match with replacement (i.e., whether each
control unit can be matched to more than one treated unit),
default=FALSE.  Use this to reduce bias when the ratio of control
units that are comparable to the treated units is small and when
pre-treatment covariates between treatment and control units diverge
substantially.
  
\textbf{Ratio Matching.}  \texttt{ratio} specifies the number of
...
\textbf{Counter.} \texttt{counter} specifies whether a counter
...
\paragraph{Propensity Score Matching} When insufficient exact matches
can be found, as this becomes increasingly common as the number of
covariates increase, we need to find a way to identify matches that
are ``close.''  In this situation, matching on the estimated
propensity score is a useful alternative.  The propensity score is the
probability that a unit receives treatment, given the covariates.  To
conduct propensity score matching, with pre-treatment covariates
composed of real earnings in 1974 and 1975:
...
\paragraph{Propensity Score Matching with Exact Restriction}
...
\paragraph{Mahalanobis Metric Matching within Caliper}
...
\paragraph{Mahalanobis Metric Matching}
...
\subsubsection{Options}
Optimal matching can be performed with \MatchIt\ using an add-on
package called ``\texttt{optmatch}'' (\cite{Hansen04}), which must be
installed separately by typing at the R command prompt,
\begin{verbatim}
> install.packages("optmatch", contriburl = "http://www.stat.lsa.umich.edu/~bbh/optmatch")
\end{verbatim}
Currently, \texttt{optmatch} is not available for Windows.  For more
...
Once \texttt{optmatch} is installed, optimal matching can be chosen
within \MatchIt\ simply by setting \texttt{m.order=1}.  All other
options for nearest neighbor matching remain the same.

Here we provide an example of optimal matching.  First we show the
default greedy matching, which matches the treated students in order
from the treated unit with the highest propensity score to the treated
unit with the smallest propensity score.
...
% Difference in matched pairs about the same for both methods, about 0.09
%temp <- foo$data$pscore[foo$data$treat==1]-foo$data$pscore[foo$match.matrix[,1]]
%mean(abs(temp))
