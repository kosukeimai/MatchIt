...
matching method in \MatchIt\ is what is known as ``greedy" matching, where the treated units 
are matched
one at a time, with the closest control match for each chosen, without trying to minimize a global distance measure.  
Another method, known as ``optimal" matching, finds the matched samples with the smallest average distance between each 
matched pair.   Optimal matching can be performed with \MatchIt\ using an add-on package called ``\texttt{optmatch}" 
(\cite{Hansen04}).  Because \texttt{optmatch} is not yet available on CRAN, it must be installed separately, from
\hlink{FILL IN HERE}{http://gking.harvard.edu/zelig}.  

Once \texttt{optmatch} is installed, optimal matching can be 
chosen within \MatchIt\ by simply setting \texttt{m.order=1}.  All
other options for nearest neighbor matching remain the same.  Here we provide an example of optimal matching.  First
...
In this example, we see that the resulting balance in the matched samples is better when optimal matching is used.
For example, the t-statistic of the propensity score in matched samples is 0.14 with greedy matching and only 
0.03 with optimal matching.  With large control pools, greedy and optimal matching may lead to very similar (or the same) 
sets of matches and research (\cite{GuRos93}) has shown that greedy and optimal matching generally choose 
...
treated units.  Full matching is optimal in terms of minimizing a weighted average of the distances between
each treated subject and each control subject within each stratum.  Full matching can be implemented in \MatchIt\ using
the add-on package ``optmatch", discussed in Section \ref{optmatch}.  All options discussed in the documentation with
``optmatch" can be used in \MatchIt\ through the ``????" option.
...
The default is not to do full matching (\texttt{full=FALSE}).  
