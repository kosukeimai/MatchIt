...
\section{Statistical and Command Overview}
...
%%% (K) talk briefly about what matching is.

\texttt{matchit()} implements a variety of matching procedures, for
which examples and details on finetuning the matching procedures are
described in Section~\ref{methods} and Section~\ref{sec:reference}. A
general syntax is
\begin{verbatim}
...
\texttt{summary()} summarizes the resulting balance of covariates
after matching.
  
\texttt{plot()} creates plots to assess balance of covariates.
...
race and education. Now, proceed to
Section~\ref{subsubsec:exact-balance} for checking the balance of
covariates after exact matching.
...
\subsubsection{Subclassification}
\label{subsubsec:subclass-balance}
...
\subsubsection{Nearest Neighbor Matching}
\label{subsubsec:nearest-balance}
...
\subsubsection{Optimal Matching}
\label{subsubsec:optimal-balance}
