...
\item \texttt{m.order}  specifies the order in which to match
  ...
  \begin{itemize}
  \item ``2'' indicates matching from the largest value of the
    distance measure to the smallest (default).
  \item ``3'' indicates matching from the smallest value of the
...
\item \texttt{replace} specifies whether each control unit can be
  ...
  as a match only once (``without replacement"),
  \texttt{replace=FALSE} (default).
\item \texttt{exact} specifies variables on which to perform exact
  ...
  vector of variable names \texttt{exact=c("X1", "X2")} that are names
  of variables in \texttt{data}.
...
The following example script can be run by typing {\tt demo(nearest)}
at the R prompt.
