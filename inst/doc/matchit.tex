...
``1-1'' match).  The average propensity scores in the matched treated and
...
samples.  The {\tt summary()} command gives further
...
This reveals simple statistics of the propensity score and
the covariates used in the propensity score specification for the full
and matched samples, including t-statistics and balance bias
statistics used to assess whether there was a reduction in bias in the covariates.   All
...
$0.011$ standard deviations in the matched samples.  
More specifically, job training
...
\$96 (t-statistic=0.32) in 1975. This one-to-one
...
\begin{figure}[h]
  \begin{center}
    \rotatebox{270}{\includegraphics[height=2.35in,angle=0]{figs/f2figa}}
    \rotatebox{270}{\includegraphics[height=2.35in,angle=0]{figs/f2figb}}
    \hfill
...
The command {\tt summary(foo3)\$sum.all} gives a summary of the covariates before
matching, whereas \\ {\tt summary(foo3)\$sum.matched} gives the comparable
...
\begin{figure}[tbp]
  \begin{center}
    \rotatebox{270}{\includegraphics[height=2.35in,angle=0]{figs/subclass1}}
    \rotatebox{270}{\includegraphics[height=2.35in,angle=0]{figs/subclass2}}
    \rotatebox{270}{\includegraphics[height=2.35in,angle=0]{figs/subclass3}}
    \rotatebox{270}{\includegraphics[height=2.35in,angle=0]{figs/subclass4}}
    \hfill
...
that have t-statistics greater than $2$ in any subclass (the default of $2$ can be changed by specifying the {\texttt sig} option
in the {\texttt summary()} command).
...
\item \textbf{Reestimation.} \texttt{reestimate} specifies whether the
  ...
  reasons, especially if many  units were not matched so that the matched samples are quite different from the original samples.
...
\item \textbf{Replacement matching.} \texttt{replace} is a logical vector which specifies whether
  to match with replacement (i.e., whether each control unit can be matched to more than one treated unit), 
  default=FALSE.  Use this to reduce bias
...
\item \textbf{Ratio Matching.}  \texttt{ratio} specifies the number of
  control units to be matched to each treatment unit, default=1.  To increase efficiency of the estimand when there 
  is a large number of comparable control units available, use
  larger integer values. If
  ...
  there are not enough control units for the specified method), then the higher
...
\item \textbf{Counter.} \texttt{counter} specifies whether a counter
  indicating the progress of the matching is displayed.
...
The following are the outputs of each \texttt{matchit} object; if outputs are not applicable for the original
\texttt{matchit} call, they are returned as \texttt{NA}:
...
\item \texttt{match.matrix} is an $n_1$ by \texttt{ratio} data frame,
 ...
 \item \texttt{in.sample} is a vector of length $n$ that displays
   whether the units were eligible for matching due to discarding due to common support restrictions.  If
 ...
 \item \texttt{psweights} is a vector of length $n$ that
...
\item \texttt{treat} stores the treatment indicator from \texttt{data} (the left-hand side of the assignment model).

\item \texttt{covariates} stores the covariates used in the right-hand side of the
  assignment model
\end{enumerate}
...
\item \texttt{sum.all} is a data frame which contains variable names
  ...
  \begin{itemize}
  \item means of all covariates $X$ for treated and control units, where \texttt{Means Treated}$= \mu_{X|T=1} =
    \frac{1}{n_1} \sum_{T=1} X_i$ and
    \texttt{Means Control}$= \mu_{X|T=0} = \frac{1}{n_0} \sum_{T=0} X_i$,
  \item standard deviations for all covariates $X$, where \texttt{SD}$= s_X = \sqrt{\frac{1}{n-1}
  ...
  \item matchit bias statistics \texttt{Bias}$=\frac{\mu_{X|T=1} -
      \mu_{X|T=0}}{s_{x|T=1}}$, \\ where $s_{x|T=1} = \sqrt{\frac{1}{n_1-1}
...
\item \texttt{sum.matched} is a data frame  
  ...
  \item weighted standard deviations for all covariates $X$ and their
    ...
    \texttt{SD} $= s_{wX} = \sqrt{\frac{1}{n} \sum_{i} (w_iX_i - \overline{wX})^2}$,
  \item t-statistics (weighted by {\texttt psweights}) from means tests of all covariates $X$ and their
...
\item \texttt{q.table} is an array that contains the same
  information as \texttt{sum.all} and \texttt{sum.matched} by subclass

\item \texttt{qn} gives the sample sizes in the full and matched samples, by subclass
...
{\tt match.data(match.out, "control")} gives just the matched control units.  
...
calculates a simple difference in means and corresponding variance estimate.  The estimated treatment effect is:
\begin{equation}
\label{ate} 
...
\href{http://gking.harvard.edu/zelig/}.
...
potential outcomes.  For example, for the treated group, the outcomes under control,
$Y(0)$, are missing, whereas
the outcomes under treatment, $Y(1)$, are observed.  We impute
...
$Y_i(1)-\widehat{Y_i(0)}$, is straightforward. The average treatment effect for
...
at \\ {\url gking.harvard.edu/zelig}.
\begin{verbatim}
> library(Zelig)
> match.out1 <- matchit(treat ~ age + educ + black + hispan + nodegree +
                      married + re74 + re75, data = lalonde)
...
Next, set the $x$ variables to the covariates observed in the matched treated units:
\begin{verbatim}
...
\begin{itemize}
\item \texttt{formula}: a symbolic representation of the propensity score
  model estimated, in the form {\tt Y $\sim$ X1 + X2 + \dots}.  Exact matching is done on the variables on the right-hand side of \texttt{formula}.
\item \texttt{data}: a data frame containing the variables in the formula.
\item \texttt{counter}: an indicator for whether a counter showing the percent
  of treated units matched so far is printed out, default={\tt TRUE}.
\end{itemize}
