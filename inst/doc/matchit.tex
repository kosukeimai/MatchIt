...
be similar.
%% the second half of this paragraph be moved?
%%
%and this can be checked using the \texttt{summary()}
%command after subclassification (see Section~\ref{subsec:balance}).
%One can then apply parametric analysis within each subclass, rather
%than the entire dataset, while still summarizing the results for the
%entire dataset (see Section~\ref{subsec:analysis}.  This can
...
%than global parametric assumptions.
...
Call: matchit(formula = treat ~ age + educ + black + hispan + married + nodegree +
              re74 + re75, data = lalonde, method = "full", distance = "logit")
...
## a short summary
> m.out
Call: matchit(formula = treat ~ re74 + re75 + age + educ, data = lalonde, method = "optimal", 
              distance = "logit", ratio = 2) 
