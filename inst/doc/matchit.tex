...
every treated unit to all possible control units with the same
covariate values.  For example, to find exact matches on
...
matched units.  The matching forms subclasses based on the covariates
in the formula portion of the \MatchIt\ call; within each subclass,
all units have the same covariate values.  To obtain basic information
...
The means of the race variables ({\tt black} and {\tt hispan}) are
quite different between the full treated and control groups.  Since
covariates are the same within each matched subclass, means tests are
not reported for the matched data.\footnote{Overall means are also not
  reported.  These means are not necessarily the same between control
  and treated groups as the number of treated and control units in
  each subclass may differ.}  The two binary covariates here form
three subclasses, and we see, for example, that 243 units (156 treated
and 87 control) are black and non-hispanic.
...
matched samples, including t-statistics and balance bias
statistics.\footnote{Balance bias statistics are a measure of how
  close covariates between treated and control groups are.  For a
  formal definition, see Subsection~\ref{cmd:sum}.}  The last column
of the summary statistics for the matched data reports whether there
there was a reduction of balance bias in the covariates with matching,
denoted by ``1'' if yes, and 0 otherwise.  All three variables
(propensity score, 1974 income, and 1975 income) had reductions in
bias due to the matching.  For example, the original bias in 1974
income was $-0.72$ standard deviations, but is only $0.011$ standard
deviations in the matched samples.  More specifically, job training
...
\$96 (t-statistic=0.32) in 1975.  This one-to-one matching algorithm
has thus chosen 185 control individuals who do look very similar to
the treated group on the covariates used in the matching process (1974
income and 1975 income).
