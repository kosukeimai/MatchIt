...
columns give the median, mean, and maximum differences between quantiles of treated and control covariates.  If the empirical 
distributions of
...
file="figs/qqplotnn1.pdf"
pdf(file)
plot(m.out, interactive=FALSE, which.xs=c("re74", "re75", "educ"))
cat("\\includegraphics[width=2.75in, height=2.75in]{", file, "}\n\n", sep="")
@
<<echo=FALSE, results=hide>>=
dev.off()
@
<<results=tex, echo=FALSE>>=
file="figs/qqplotnn2.pdf"
pdf(file)
plot(m.out, interactive=FALSE, which.xs=c("black", "hispan", "age"))
cat("\\includegraphics[width=2.75in, height=2.75in]{", file, "}\n\n", sep="")
@
<<echo=FALSE, results=hide>>=
dev.off()
