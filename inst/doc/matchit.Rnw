...
of unit $i$ under treatment and $y_i(0)$ the potential outcome of unit
$i$ under control.  The variables $y_i(1)$ and $y_i(0)$ are jointly
...
group, the potential outcomes under control, $Y_i(0)$, are missing,
whereas the outcomes under treatment, $Y_i(1)$, are observed, and the
goal of the analysis is to impute the missing outcomes, $Y_i(0)$ in
...
individual $i$'s treatment effect is $Y_i(1)-\widehat{Y}_i(0)$ where
$\widehat{Y}_i(0)$ is a Monte Carlo estimate of the average missing
...
option was specified) because they are outside the range of common support.
