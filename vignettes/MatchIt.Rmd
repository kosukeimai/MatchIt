...
The first argument is a `formula` relating the treatment to the covariates used in estimating the propensity score. The `data` argument specifies the dataset where these variables exist. The `method` argument specifies the method of matching to be performed (here, `"nearest"` for nearest neighbor matching). The `distance` argument specifies the method for estimating the propensity score (here, `"glm"` for generalized linear model, which implements logistic regression by default)[^1]. Many other arguments are available for tuning the matching method and method of propensity score estimation.

[^1]: Note that setting `distance = "logit"`, which was the default in `MatchIt` version prior to 4.0.0, will also estimate logistic regression propensity scores. Because it is the default, the `distance` argument can actually be omitted if logistic regression propensity scores are desired.
...
Next is a table of the percent balance improvement after matching; ideally these values would be close to 100% (and no more!), indicating that all of the initial imbalance has been removed. Some early matching theory relied on these values [@cochran1973; @rubin1973a].
...
We can then estimate a treatment effect in this dataset using the standard regression functions in R, like `lm()` or `glm()`, being sure to include the matching weights (stored in the `weights` variable of the `match.data()` output) in the estimation[^2]. We recommend using cluster-robust standard errors for most analyses, with pair membership as the clustering variable; the `lmtest` and `sandwich` packages together make this straightforward.

[^2]: With 1:1 nearest neighbor matching without replacement, excluding the matching weights does not change the estimates. For all other forms of matching, they are required, so we recommend always including them for consistency.
