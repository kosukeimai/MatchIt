...
We can then estimate a treatment effect in this dataset using the standard regression functions in R, like `lm()` or `glm()`. We recommend using robust standard errors for most analyses; the `lmtest` and `sandwich` packages together make this straightforward.
...
Unlike 1:1 matching without replacement, full matching produces a set of weights that must be included in the estimation of the treatment effect. It is the application of these weights that creates balance in the matched sample. (See `?matchit` for information on how the weights are computed). When using matching weights, a robust (sandwich) standard error must be used. A variety of R packages produce robust standard errors for weighted regression, often relying on functions in the `sandwich` or `survey` R packages; as before, we'll use the `lmtest` and `sandwich` packages here because they provide a fairly general interface to estimating coefficients and standard errors.
