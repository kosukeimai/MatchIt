...
opt <- requireNamespace("optmatch", quietly = TRUE)
...
Given the poor performance of nearest neighbor matching in this example, we can try a different matching method or make other changes to the matching algorithm or distance specification. Below, we'll try full matching, which matches every treated unit to at least one control and every control to at least one treated unit [@hansen2004; @stuart2008a]. We'll also try a different link (probit) for the propensity score model. `r if (!opt) notice`

```{r, include=FALSE}
#In case optmatch goes offline, don't run lines below
if (!opt) knitr::opts_chunk$set(eval = FALSE)
```
```{r}
...
```{r, include = FALSE}
est <- {
  if (opt) coeftest(fit2, vcov. = vcovCL, cluster = ~subclass)
  else matrix(c(2091.547, 678.3335, 0.002140147), nrow = 1,
              dimnames = list("treat", c("Estimate", "Std. Error", "Pr(>|t|)")))
}
```
...
>
> The estimated effect was \$`r round(est["treat", "Estimate"])` (SE = `r round(est["treat", "Std. Error"], 1)`, p = `r round(est["treat", "Pr(>|t|)"], 3)`), indicating that the average effect of the treatment for those who received it is to increase earnings.
