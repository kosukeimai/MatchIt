...
Note that not all methods can involve sampling weights in the estimation. Some propensity score estimation methods, like `randomForest` and `bart` (as presently implemented), cannot incorporate sampling weights; one alternative is to include the sampling weights as a predictor in the propensity score model. Sampling weights should still be supplied to `matchit()` even when using these methods to avoid having to use `add_s.weights()`.
